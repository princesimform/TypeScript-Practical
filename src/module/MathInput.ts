...
const changeValidDecimal = (validDecimalValue: boolean): void => { validDecimal = validDecimalValue }
const getValidDecimal = (): boolean => { return validDecimal }

function operand(val: string): void {
    output.value += val;
}

function operator(value: string): void {
    if (value != '=') {
        output.value += value;
    }
    validDecimal = false;
